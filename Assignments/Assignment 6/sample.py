...
def gradLinearReg(X,y,alpha=0.05,maxIter=15000):
    thetas = []
    theeta0list = []
    theeta1list= []
    costs = [];
...
def calcualteModelandPlotonTrainData():
    ...
    thetas, theeta0list, theeta1list, costs = gradLinearReg(X,y, float(alpha))
...
def writeToThetaFile(filename, thetalist, costs, thetavalue):
     with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
...
writeToThetaFile("CostFunction_Theta0.csv", theeta0list, costs, "Theta0_Value")
writeToThetaFile("CostFunction_Theta1.csv", theeta1list, costs, "Theta1_Value")